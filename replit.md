...
- **Category-aware Smart Metadata + Document Map**: Each new document is classified into one of 8 categories and profiled into standard + contextual key-value facets via a 2-pass LLM (classify→extract), persisted as `DocumentFacet` rows plus `Document.doc_category`/`doc_category_confidence`/`smart_metadata`. Extraction is non-fatal (never blocks indexing) and idempotent under concurrency — the replace-facets write section is serialized with a per-document PG advisory lock, backed by a unique `(document_id, key, value_norm)` constraint. The Document Map UI offers faceted filtering (AND/OR), a D3 facet graph, list view, reprocess, and an "Ask IDA about these" handoff to chat. The `/facets` browse view applies `min_count=2` (hides singleton facet values that can't group a set — e.g. hyper-specific topics, unique-per-doc titles); searching or drilling into a single key keeps `min_count=1` so a specific entity stays findable. The `topic` field is prompt-constrained to ≤5 broad reusable themes to reduce facet noise. The Document Map's primary purpose is to let IDA pre-filter *which* files in a knowledge base to read with per-document PageIndex (`search_document`), since PageIndex has no across-documents step. The agent tool `query_document_map` therefore (a) is auto-scoped to the conversation's attached document set — tri-state: non-empty attachment → that set, attachment resolving to zero docs → none, no attachment → whole library (for discovery) — threaded from the route via `has_attached_scope`; and (b) applies the same singleton denoising as the UI in `list_facets`. The agent prompt (both `SYSTEM_PROMPT` Case A and the dynamic in-scope block in `_build_messages`) instructs IDA: for a handful of attached files search all; for a large attached set / knowledge base, narrow with `query_document_map` first, then read only the relevant subset.
- **Background (Re)extraction Jobs with live progress**: Bulk metadata (re)extraction runs as a tracked background job (`MetadataJob` table: status/total/processed/failed/skipped/current/error/label/scope). `POST /api/metadata/extract-bulk` accepts a scope (`all`|`missing`), `document_ids`, and/or `tag_ids`, resolves the owner-scoped document set, creates the job, launches an async runner (`metadata/job_runner.py` with its own DB session, per-document failure isolation, and frequent progress commits), and returns `{job_id, total}` immediately. The UI polls `GET /api/metadata/jobs/{job_id}` for a live progress bar (X of N, processed/failed/skipped, current doc). Per-document failures are persisted as a capped JSON list in `MetadataJob.failures` (filename + error) and surfaced as a collapsible "N files failed — see why" list in both the Document Map and Knowledge Base UIs. `GET /api/metadata/jobs` lists recent jobs. Note: Alpine progress bars use object-form `:style="{width: pct+'%'}"` — string-form `:style` overwrites the static inline style and the bar never fills.
...
- **Tag dedup, central management + merge**: Tag creation is case-insensitive and reuses an existing same-name tag. Uniqueness per `(owner_id, lower(name))` is enforced at the application layer via a transaction-scoped PostgreSQL advisory lock (`lock_tag_name` in `session.py`, `pg_advisory_xact_lock(hashtext(...))`) wrapped around every tag-name get-or-create / rename path (`tags.py` `create_tag` + `update_tag` rename, `auto_tagger.apply_tags` (locks pre-acquired in sorted order to avoid deadlocks), `youtube.py` default-tag create). A DB unique index is deliberately **not** used: Replit's publish flow replicates dev unique indexes to prod *before the app boots*, so legacy prod duplicates would abort the deploy before `_migrate_dedupe_tags` could clean them. The startup migration `_migrate_dedupe_tags` merges any pre-existing duplicates (keeping the earliest) idempotently on boot in both dev and prod. `POST /api/tags/{tag_id}/merge` consolidates source tags into a target, relinking document/conversation associations with conflict-safe `on_conflict_do_nothing` upserts before deleting the sources. The Knowledge Base UI shows named tag chips and a "Manage Tags" modal (search/rename/recolor/delete/merge).
